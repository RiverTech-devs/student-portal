...
- **Riven (teacher terminal)** — local regex-NLP assistant in `portal/index.html` (`_executeNaturalLanguage` pipeline: smalltalk → normalize → pronouns → entities → intent → execute). Supports reads (student/balance/history/attendance incl. aggregate "which students...", enrollments, class catalog, tenure, historical balance from the RTC ledger, comparisons), mutations (award/deduct/transfer RTC, notes) with generic undo, and HONEST declines for unsupported actions (attendance writes, rosters, banking) — never fall back to the student card when an action verb is present. The canonical regression suite is `../debug-tools/nlp-stress.js` (extracts the real methods from this file; run it after ANY Riven change and add cases for new intents).
