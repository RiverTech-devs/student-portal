...
- **RTC economy** — students earn RTC for skill mastery (5 RTC auto-awarded on first transition to `mastered`), assignment completion (tiered by score), and arcade play. They spend on cosmetics, privileges, and IRL store items. **Duplicate-prevention** via UNIQUE constraint on `(reference_id, reference_type, user_id)` in `rtc_transactions`.
...
- **Riven (teacher terminal)** — local NLP assistant in `portal/index.html` (confidence-scored regex matcher + client-side MiniLM embedding fallback via transformers.js for free-form phrasing; near-tie between two write intents asks a clarifying question) (`_executeNaturalLanguage` pipeline: smalltalk → confirmation gate → normalize → pronouns → entities (class matched before student; class words stripped) → intent → execute). READS: student/balance/history/attendance (incl. aggregates), grades (per student/class + compare), enrollments, rosters, class catalog, contact info, tenure, historical balance, notes. WRITES — now also **assignments** (`CREATE_ASSIGNMENT`), **announcements** (`ANNOUNCE` — blast-radius, confirms with a recipient count, school-wide is admin-only), **activity enrollment** (`ACTIVITY_ENROLL`/`ACTIVITY_UNENROLL`, requires an activity word so class enrollment isn't stolen) and **facility booking** (`BOOK_FACILITY`, reuses the `check_booking_conflict` RPC and lands as `pending`). A write suppressed by a speculative cue (negated / conditional / deliberative / question) returns `SPECULATIVE_WRITE` and explains itself instead of falling through to a student card. (all undoable via the `_pushUndo` stack; "undo" walks back repeatedly): RTC award/deduct/transfer, notes, attendance marks (delete-then-insert mirror of saveClassAttendance; primary-teacher-or-admin per RLS), enroll/unenroll (reactivate-or-insert / status='removed'), class create/deactivate, grades via the security-definer RPCs (set_class_grade_override / set_academic_grade_override / upsert_quarter_grade + calculate_quarter_grades), contact info on user_profiles. SENSITIVE writes pause at `_requestConfirmation` (buttons or typed yes/no; anything else sets the action aside). Permission mirror: `_rivenCanManageClass` (admin all; teacher own class; secondary teacher everything except attendance writes). Same-named classes are resolved via the student's enrollment (`_rivenResolveClassForStudent`). Chat persists in localStorage per user (`riven_chat_<id>`, 120-message cap, Clear chat button). **Three harnesses, run ALL after any Riven change** (each extracts the real methods from this file, so neither can drift from shipped code):
  - `debug-tools/nlp-stress.js` — the recall suite: does each phrasing reach the right intent (~322 checks). NOTE: it currently reports ~9 pre-existing failures; that's the known baseline, not something you introduced. Compare against it, don't assume green.
  - `debug-tools/frontdoor-precision.js` — the **write-precision** gate. 111 items in five buckets: A in-scope commands, B reads/out-of-scope, C adversarial look-alikes (over-firing), **D guard overreach** (over-blocking), E context-carrying + compound. **Everything must stay at 100%; it exits non-zero otherwise.**
    - Bucket D is the one that matters most when you touch a guard. It is real commands that innocently carry cue vocabulary ("give eli 5 rtc he never gives up", "don't forget to give eli 5 rtc", "mark charlotte present after lunch"). The first version of this bench shipped without D, and the guards it motivated silently blocked four real commands. **A guard bench needs both directions.**
    - Three precision metrics, all gated: flat write-precision, **IMMEDIATE-write precision** (writes whose executor never calls `_requestConfirmation` — currently ADD_NOTE / ADD_SHOP_ITEM / DELETE_NOTE — these reach the database with nothing in front of the teacher), and **blast-radius precision** (whole-class or destructive intents). The immediate/executor map is derived from `portal/index.html` at runtime, not hand-maintained.
    - **Fix over-firing** by making the matcher abstain more (negative cues, tighter anchors). **Fix over-blocking by SCOPING a guard** — bind the cue to a command verb, exempt note content — never by deleting the guard. Over-deferral costs a rephrase; over-firing changes a balance or a grade.
    - Scope is the regex tier only. Phrasings the regex tier deliberately leaves to the semantic tier (e.g. a catalog edit that doesn't name the shop) are out of scope, not bugs.
    - This exists because two other deterministic front-doors in the portfolio (PIE's, ulcagent's) shipped green on their unit suites with real false positives that only a bench like this found — and a third (densanon-engines Engine A) turned out to post a journal entry on 11 of 12 adversarial sentences.
  - `debug-tools/undo-durability.js` — Riven's undo survives a reload only for call sites that pass a serializable **spec** to `_pushUndo(desc, fn, spec)`. This exercises push → persist → drop memory → restore → run, plus the 12h staleness cutoff and corrupt payloads. **Add a spec (and a reverser in `_rivenUndoReversers`) for any new write worth undoing after a refresh**; a spec-less call site still works, it just doesn't survive.
  - `debug-tools/semantic-coverage.js` — how much of the MiniLM example bank the DETERMINISTIC tier handles alone (currently 82.8%). That number is the cost of dropping the LLM layers. **Patterns must be written against NORMALIZED text** — `_normalizeInput` collapses dock/penalize→remove, reward/give→add, gold→rtc, check→show, so a pattern written against the raw phrasing silently never fires.
  - `debug-tools/recovery-ladder.js` + `debug-tools/phrasebook.js` — the model-free fallback. A miss ranks candidate intents by content-token overlap against that same bank and offers them as taps; tapping teaches the phrase SHAPE (names stripped) and after two confirmations it routes directly. **Only READ intents are ever learned** — a phrase must never start writing because it was tapped twice.
  - **Coverage telemetry**: `/coverage` shows a teacher their own numbers; admins also get the whole school from the `riven_usage` table (migration `zzzzzzzz_riven_usage_rollup.sql`). **Raw phrasings never leave the device** — only counts and entity-stripped shapes, enforced by a CHECK constraint on `riven_usage.shape` AND asserted in the phrasebook harness. If you add a new telemetry field, that assertion is the thing to extend.
  - **Riven's undo stack is `_rivenUndoStack`, NOT `_undoStack`** — the latter is the Class Sheets spreadsheet undo on the same app object. They pushed incompatible shapes and silently ate each other's entries. Don't merge them.
  - Two escaping traps when editing the pattern table from a script: writing `\b` through a shell-heredoc→Python layer turns it into a **backspace character** (use a raw `r'''…'''` string), and the resulting regex still *parses*, so only a behavioural check catches it. Prefer editing the table directly.
