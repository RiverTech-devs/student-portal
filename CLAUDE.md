...
- **Riven (teacher terminal)** — local NLP assistant in `portal/index.html` (confidence-scored regex matcher + client-side MiniLM embedding fallback via transformers.js for free-form phrasing; near-tie between two write intents asks a clarifying question) (`_executeNaturalLanguage` pipeline: smalltalk → confirmation gate → normalize → pronouns → entities (class matched before student; class words stripped) → intent → execute). READS: student/balance/history/attendance (incl. aggregates), grades (per student/class + compare), enrollments, rosters, class catalog, contact info, tenure, historical balance, notes. WRITES (all undoable via the `_pushUndo` stack; "undo" walks back repeatedly): RTC award/deduct/transfer, notes, attendance marks (delete-then-insert mirror of saveClassAttendance; primary-teacher-or-admin per RLS), enroll/unenroll (reactivate-or-insert / status='removed'), class create/deactivate, grades via the security-definer RPCs (set_class_grade_override / set_academic_grade_override / upsert_quarter_grade + calculate_quarter_grades), contact info on user_profiles. SENSITIVE writes pause at `_requestConfirmation` (buttons or typed yes/no; anything else sets the action aside). Permission mirror: `_rivenCanManageClass` (admin all; teacher own class; secondary teacher everything except attendance writes). Same-named classes are resolved via the student's enrollment (`_rivenResolveClassForStudent`). Chat persists in localStorage per user (`riven_chat_<id>`, 120-message cap, Clear chat button). **Two harnesses, run BOTH after any Riven change** (each extracts the real methods from this file, so neither can drift from shipped code):
  ...
  - `debug-tools/frontdoor-precision.js` — the **write-precision** gate: of the sentences Riven decides to WRITE on, how many were genuinely commands. 78 items in three buckets, the third being adversarial look-alikes (deliberative / negated / conditional / prose-about-the-mechanism / no-referent). **Must stay at 100% precision; it exits non-zero otherwise.** Fix a false positive by making the matcher ABSTAIN more (negative-cue guards, tighter anchors) — never by loosening a read. Over-deferral costs a rephrase; over-firing changes a balance or a grade. This exists because two other deterministic front-doors in the portfolio (PIE's, ulcagent's) shipped green on their unit suites with real false positives that only a bench like this found.
