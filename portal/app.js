    ...
    modal = h('div', { id: 'editNoteModal', class: 'modal-overlay', tabindex: '-1' }, [
      h('div', { class: 'modal', style: 'max-width:500px' }, [
        h('div', { class: 'modal-head' }, [
          h('h3', {}, 'Edit Note'),
          h('span', { class: 'spacer' }),
          h('button', { class: 'btn link danger small', onclick: closeEditNoteModal }, 'Close')
        ]),
        h('div', { id: 'editNoteBody', class: 'modal-body' }, '')
      ])
    ]);
    document.body.appendChild(modal);
    modal.addEventListener('click', (e) => { if (e.target === modal) closeEditNoteModal(); });
    window.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeEditNoteModal(); });
  }
...
function renderEditNoteForm(note, classId, students, filters, userRole, userId) {
  ...
  let selectedSentiment = note.sentiment;
  ...
  const sentimentGroup = h('div', { class: 'col', style: 'gap:4px' }, [
    ...
    h('div', { class: 'row', style: 'gap:12px' }, sentimentOptions.map(opt =>
      h('label', { style: `cursor:pointer; color:${opt.color}` }, [
        h('input', {
          type: 'radio',
          name: 'editNoteSentiment',
          value: opt.value,
          checked: opt.value === note.sentiment,
          onchange: (e) => {
            selectedSentiment = e.target.value;
            updateEditValueField();
          }
        }),
        ` ${opt.label}`
      ])
  ...
  // Category dropdown
  const categorySelect = h('select', { id: 'editNoteCategory', onchange: (e) => {
    selectedCategory = e.target.value;
    editCustomCategoryWrap.style.display = e.target.value === 'custom' ? 'block' : 'none';
  }}, categoryOptions.map(opt =>
    h('option', { value: opt.value, selected: opt.value === note.category }, opt.label)
  ));

  const editCustomCategoryInput = h('input', {
    type: 'text',
    id: 'editNoteCustomCategory',
    placeholder: 'Enter custom category',
    value: note.custom_category || ''
  });
  const editCustomCategoryWrap = h('div', { style: note.category === 'custom' ? 'display:block' : 'display:none' }, [editCustomCategoryInput]);

  const categoryGroup = h('div', { class: 'col', style: 'gap:4px' }, [
    h('label', { class: 'muted', for: 'editNoteCategory' }, 'Category'),
    categorySelect,
    editCustomCategoryWrap
  ]);

  // Value field
  const editValueInput = h('input', {
    type: 'number',
    id: 'editNoteValue',
  ...
  const editValueHint = h('small', { id: 'editNoteValueHint', class: 'muted' }, '');
  const valueGroup = h('div', { class: 'col', style: 'gap:4px' }, [
    h('label', { class: 'muted', for: 'editNoteValue' }, 'Value'),
    editValueInput,
    editValueHint
  ]);

  function updateEditValueField() {
    const hint = document.getElementById('editNoteValueHint');
    const input = document.getElementById('editNoteValue');
    if (selectedSentiment === 'neutral') {
      input.disabled = true;
      input.value = '';
      hint.textContent = 'Neutral notes have no value';
  ...
  // Note text area
  const editNoteTextarea = h('textarea', {
    id: 'editNoteText',
  ...
  const noteGroup = h('div', { class: 'col', style: 'gap:4px' }, [
    h('label', { class: 'muted', for: 'editNoteText' }, 'Note'),
    editNoteTextarea
  ]);
  ...
  const saveBtn = h('button', {
    ...
    onclick: async () => {
      const sentiment = selectedSentiment;
      const category = selectedCategory;
      const customCategory = category === 'custom' ? (editCustomCategoryInput.value.trim() || null) : null;
      ...
      if (sentiment !== 'neutral' && editValueInput.value !== '') {
      ...
      const { error } = await sb.from('student_notes').update({
        sentiment,
        category,
        custom_category: customCategory,
        value,
